// Mock data for Julia's Secretary account.
// Persona: Julia Chen — runs two restaurants (New Wok Norbury, New Wok Plaistow),
// owns a flat at 12 Plaistow Road #2, drives a Mercedes LT21 ABC.
...
// Categories (folder taxonomy applied across all entities)
const CATEGORIES = [
...
// status: due_soon | overdue | scheduled | done | drafting | needs_review
// Items now also carry: category, date (for time-slicing), tags
const ITEMS_SEED = [
  ...
  { id: 'i3', entity: 'newwok-norbury', category: 'tax', type: 'Business rates', title: 'Q2 business rates demand', amount: 1840, dueDate: '2026-06-01', date: '2026-04-29', issuer: 'Croydon Council', ref: 'BR-7740-22', status: 'scheduled', confidence: 0.98, capturedAt: '2026-04-29 11:02', preview: 'rates', tags: ['rates'] },
  ...
  // Norbury historical fill — finance/utilities/operations/legal across quarters
  { id: 'n1', entity: 'newwok-norbury', category: 'utilities', type: 'Utility', title: 'EDF electricity — Mar', amount: 712, status: 'done', date: '2026-03-10', paidAt: '2026-03-25', issuer: 'EDF Energy', confidence: 0.97, capturedAt: '2026-03-10 12:00', preview: 'gas' },
  { id: 'n2', entity: 'newwok-norbury', category: 'utilities', type: 'Utility', title: 'EDF electricity — Feb', amount: 690, status: 'done', date: '2026-02-08', paidAt: '2026-02-25', issuer: 'EDF Energy', confidence: 0.97, capturedAt: '2026-02-08 09:14', preview: 'gas' },
  { id: 'n3', entity: 'newwok-norbury', category: 'finance', type: 'Invoice', title: 'Sea Harvest — fish supplier', amount: 1240, status: 'done', date: '2026-04-12', paidAt: '2026-04-19', issuer: 'Sea Harvest Ltd', confidence: 0.96, capturedAt: '2026-04-12 10:00', preview: 'rates' },
  { id: 'n4', entity: 'newwok-norbury', category: 'finance', type: 'Invoice', title: 'Bidfood — dry goods', amount: 880, status: 'done', date: '2026-04-05', paidAt: '2026-04-12', issuer: 'Bidfood', confidence: 0.95, capturedAt: '2026-04-05 11:00', preview: 'rates' },
  ...
  { id: 'n7', entity: 'newwok-norbury', category: 'tax', type: 'VAT', title: 'VAT return — Q1 26', amount: 4120, status: 'done', date: '2026-04-07', paidAt: '2026-04-07', issuer: 'HMRC', confidence: 0.99, capturedAt: '2026-04-07 09:00', preview: 'hmrc' },
  ...
  { id: 'n10', entity: 'newwok-norbury', category: 'tax', type: 'Tax', title: 'Corporation tax — final notice', amount: 8900, dueDate: '2025-12-30', date: '2025-11-28', status: 'done', paidAt: '2025-12-15', issuer: 'HMRC', confidence: 0.99, capturedAt: '2025-11-28 10:00', preview: 'hmrc' },
];
...
// Reactive store — entities & items can be mutated, components subscribe.
const Store = (() => {
  let state = { entities: ENTITIES_SEED.map(e => ({ ...e })), items: ITEMS_SEED.map(i => ({ ...i })) };
  const subs = new Set();
  return {
    get: () => state,
    sub: (fn) => { subs.add(fn); return () => subs.delete(fn); },
    set: (patch) => { state = { ...state, ...patch }; subs.forEach(f => f(state)); },
    addEntity: (e) => { state = { ...state, entities: [...state.entities, e] }; subs.forEach(f => f(state)); },
    removeEntity: (id) => { state = { ...state, entities: state.entities.filter(e => e.id !== id) }; subs.forEach(f => f(state)); },
    updateEntity: (id, patch) => { state = { ...state, entities: state.entities.map(e => e.id === id ? { ...e, ...patch } : e) }; subs.forEach(f => f(state)); },
  };
})();
...
window.SEC_DATA = { get ENTITIES() { return Store.get().entities; }, get ITEMS() { return Store.get().items; }, CONTACTS, TODAY, STATUS_META, CATEGORIES, Store, useStore };
